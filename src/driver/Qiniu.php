...
class Qiniu extends File
{
    ...
    public function upload($client = false)
    {
        ...
        switch (self::$config->get('storage_qiniu_region')) {
            ...
            default:
                throw new \zongphp\exception\Exception('未配置七牛云存储区域');
        }
    }
    ...
    public function base($name = '')
    {
        ...
        switch (strtolower(self::$config->get('storage_qiniu_is_https'))) {
            ...
            case 'auto':
                return "//{$domain}/{$name}";
            default:
                throw new \zongphp\exception\Exception('未配置七牛云URL前缀');
        }
    }
    ...
    public function save($name, $content)
    {
        ...
        if ($err !== null) Log::write(__METHOD__ . " 七牛云文件上传失败，{$err->message()}");
