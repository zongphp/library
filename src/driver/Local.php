...
class Local extends File
{
    ...
    {
        return __WEB__. '?s=admin/plugs/upload';
    }
    ...
    {
        $path = $safe ? 'upload/safefile' : 'upload';
        return str_replace('\\', '/', ROOT_PATH .DS. "{$path}/{$name}");
    }
    ...
    public function save($name, $content, $safe = false)
    {
        try {
            ...
            Log::write(__METHOD__ . " 本地文件存储失败，{$e->getMessage()}");
        }
        return null;
    }
