...
class Data
{
    ...
    public static function save($dbQuery, $data, $key = 'id', $where = [])
    {
        ...
        $dbInfo =  Db::table($table,true)->where($where);
        $dbInfoSet =  Db::table($table,true)->where($where);
        if(isset($where[$key])){
            $map = [];
        }else{
            if(is_string($value)){
               
                $dbInfo->whereIn($key,explode(',', $value));
                $dbInfoSet->whereIn($key,explode(',', $value));
            }else{
                $map = [
                    [$key,$value]
                ];
        ...
        //$map = isset($where[$key]) ? [] : (is_string($value) ? [[$key, 'in', arrToInStr($value)]] : [$key => $value]);
        if (is_array($info = $dbInfo->first()) && !empty($info)) {
            if ($dbInfoSet->update($data) !== false) {
                return isset($info[$key]) ? $info[$key] : true;
            } else {
                return false;
            }
        } else {
            return Db::table($table,true)->insertGetId($data);
        }
    }
