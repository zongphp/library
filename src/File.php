...
class File
{
    ...
    public static function down($url, $force = false, $expire = 0)
    {
        try {
            ...
            Log::write(__METHOD__ . " File download failed [ {$url} ] {$e->getMessage()}");
...
try {
    ...
    Log::write(__METHOD__ . " File storage initialization exception. [{$e->getMessage()}]");
}
